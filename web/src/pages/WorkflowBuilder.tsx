...
let idSeq = 1;
const genId = (prefix) => `${prefix}-${Date.now()}-${idSeq++}`;

function WorkflowBuilderInner() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { getWorkflow, saveWorkflow, devices, workflowsLoading } = useData();
  ...
  const onConnect = useCallback((connection) => setEdges((eds) => addEdge({ ...connection, animated: false }, eds)), [setEdges]);
  ...
  if (workflowsLoading) {
    return <div className="flex flex-1 items-center justify-center text-ink-500">불러오는 중…</div>;
  }
