...
import importlib.util, json, platform, shutil, subprocess, sys, threading
...
<section id="models" class="page"><div class="card"><h2>Ollama 모델</h2><div class="row"><input id="model" placeholder="qwen3:4b"><button id="pull">설치</button><button id="refresh">조회</button></div><p id="modelMsg" class="msg"></p><div id="modelList"></div></div></section>
<section id="tools" class="page"><div class="card"><h2>승인된 Tool 카탈로그</h2><p class="muted">임의 명령 대신 아래 허용 목록만 설치합니다.</p><div id="toolList"></div></div></section>
...
async function models(){try{let d=await api('/api/models');$('#modelMsg').textContent='Ollama 연결됨';$('#modelList').innerHTML=(d.models||[]).map(m=>`<div class="item"><b>${esc(m.name)}</b><button class="danger" onclick="delModel('${esc(m.name)}')">삭제</button></div>`).join('')||'<p class="muted">설치된 모델 없음</p>'}catch(e){$('#modelMsg').textContent='Ollama 연결 실패: '+e.message;$('#modelMsg').className='msg bad'}}window.delModel=async n=>{if(confirm(n+' 삭제?')){await api('/api/models/delete',{method:'POST',body:JSON.stringify({name:n})});models()}};$('#refresh').onclick=models;$('#pull').onclick=async()=>{try{$('#modelMsg').textContent='설치 중…';await api('/api/models/pull',{method:'POST',body:JSON.stringify({name:$('#model').value})});models()}catch(e){alert(e.message)}};
async function tools(){let d=await api('/api/tools');$('#toolList').innerHTML=d.tools.map(t=>`<div class="item"><span><b>${t.name}</b><br><small class="muted">${t.installed?'설치됨':'설치 필요'}</small></span><button ${t.installed?'disabled':''} onclick="installTool('${t.id}')">설치</button></div>`).join('')}window.installTool=async id=>{if(confirm('승인된 패키지를 설치할까요?')){try{await api('/api/tools/install',{method:'POST',body:JSON.stringify({id})});tools()}catch(e){alert(e.message)}}};$('#start').onclick=async()=>{await api('/api/worker/start',{method:'POST',body:'{}'});status()};$('#stop').onclick=async()=>{await api('/api/worker/stop',{method:'POST',body:'{}'});status()};status();setInterval(status,3000);
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path in {"/api/models/pull","/api/models/delete"}:
                ...
                pull=self.path.endswith('pull');r=httpx.request('POST' if pull else 'DELETE',c["ollama_url"].rstrip('/')+('/api/pull' if pull else '/api/delete'),json={"name":name,"stream":False},timeout=None);r.raise_for_status();LOGS.append(('모델 설치: ' if pull else '모델 삭제: ')+name);self.reply({"ok":True});return
            if self.path=="/api/tools/install":
                t=next((x for x in TOOLS if x["id"]==d.get("id")),None)
                if not t:raise ValueError("승인되지 않은 Tool입니다.")
                if t["packages"]:subprocess.run([sys.executable,'-m','pip','install',*t["packages"]],check=True,timeout=600)
                if t.get("binary") and not shutil.which(t["binary"]):raise RuntimeError(t["binary"]+'는 운영체제 패키지 관리자로 설치해야 합니다.')
                LOGS.append('Tool 설치: '+t["name"]);self.reply({"ok":True});return
            self.reply({"error":"Not found"},404)
...
def serve(host='127.0.0.1',port=8765):
    if host not in {'127.0.0.1','localhost','::1'}:raise ValueError('관리 UI는 localhost에만 바인딩할 수 있습니다.')
