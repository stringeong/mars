...
import { Plus, Monitor, Laptop, Smartphone, HardDrive, X, FolderPlus, ShieldCheck, Sparkles, Trash2, Info } from 'lucide-react';
...
export default function Devices() {
  const { devices, seedDemoDevices, removeDevice } = useData();
          ...
          <div className="mb-5 flex items-start gap-3 rounded-xl2 border border-brand-200 bg-brand-50 p-4">
            ...
            <div className="flex-1">
              ...
              <p className="mt-0.5 text-[12px] leading-relaxed text-ink-600">
                ...
                지금 바로 등록하거나, 샘플 기기로 앱을 둘러볼 수 있어요.
              </p>
            </div>
        ...
        {devices.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl2 border-2 border-dashed border-brand-200 bg-brand-50/30 py-16 text-center">
            ...
            <div className="mt-5 flex gap-2.5">
              <button
                onClick={() => setShowRegister(true)}
                className="flex items-center gap-1.5 rounded-lg bg-brand-600 px-4 py-2.5 text-[12.5px] font-semibold text-white shadow-soft hover:bg-brand-700"
              >
                <Plus size={15} /> 기기 등록하기
              </button>
              <button
                onClick={seedDemoDevices}
                className="flex items-center gap-1.5 rounded-lg border border-brand-200 bg-white px-4 py-2.5 text-[12.5px] font-semibold text-brand-600 hover:bg-brand-50"
              >
                <Sparkles size={15} /> 샘플 기기로 둘러보기
              </button>
            </div>
          </div>
        ) : (
              ...
              return (
                <Card key={d.id} className="group relative p-5">
                  <button
                    ...
                    title="기기 연결 해제"
                    className="absolute right-3 top-3 rounded-md p-1 text-ink-300 opacity-0 hover:bg-rose-50 hover:text-rose-500 group-hover:opacity-100"
                  >
                    <Trash2 size={14} />
                  </button>
...
const deviceTypeOptions: DeviceType[] = ['Windows Laptop', 'Desktop PC', 'Android Phone', 'NAS Server', 'iPhone'];

function RegisterDeviceDrawer({ onClose }: { onClose: () => void }) {
  const { registerDevice } = useData();
  const [name, setName] = useState('My Device');
  const [type, setType] = useState<DeviceType>('Windows Laptop');
  const [access, setAccess] = useState<'read-only' | 'read-write'>('read-only');
  const [scope, setScope] = useState<'local-only' | 'selected-workflows' | 'all-workflows'>('selected-workflows');
  const [folders, setFolders] = useState<string[]>(['Documents']);
  const allFolders = ['Documents', 'Desktop', 'Research Papers', 'Personal Photos'];

  const toggleFolder = (f: string) => setFolders((prev) => (prev.includes(f) ? prev.filter((x) => x !== f) : [...prev, f]));
  ...
  const handleRegister = () => {
    const newDevice: Device = {
      ...
      type,
      status: 'online',
      cpuModel: type === 'NAS Server' ? 'ARM Cortex-A76' : 'Detected on connect',
      ramGb: 16,
      storage: '512 GB',
      network: 'Connected',
      // Just connected, no workflow has run on it yet — only baseline OS
      // overhead is showing, not literally 0% (a real machine never idles
      // at exactly zero).
      resources: { cpu: Math.round(1 + Math.random() * 6), ram: Math.round(6 + Math.random() * 10), ramTotalGb: 16 },
      folders,
        ...
        <div className="space-y-6 px-5 py-5">
          ...
          <div>
            ...
            <div className="space-y-2">
              {(['local-only', 'selected-workflows', 'all-workflows'] as const).map((s) => (
                <label
                  key={s}
                  className={clsx(
          ...
          <button
            ...
            className="w-full rounded-lg bg-brand-600 py-2.5 text-[12.5px] font-bold text-white hover:bg-brand-700"
          >
            Register Device
          </button>
        </div>
